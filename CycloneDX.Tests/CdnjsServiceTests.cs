...
{
    public class CdnjsServiceTests
    {
        [Fact]
        public async Task GetComponentAsync_ReturnsComponent()
        {
            ...
            mockHttp.When($"{CdnjsService.BaseUrl}/libraries/testpackage")
                            .Respond("application/json", mockResponseContent);

            var cdnjsService = new CdnjsService(mockHttp.ToHttpClient());
            var package = new LibmanPackage(LibmanProvider.cdnjs) { Name = "testpackage", Version = "1.0.2" };

            // Act
            var component = await cdnjsService.GetComponentAsync(package).ConfigureAwait(false);
        ...
        public async Task GetComponentAsync_NotFoundReturnsNull()
        {
            ...
            var service = new CdnjsService(mockHttp.ToHttpClient());
            var package = new LibmanPackage(LibmanProvider.cdnjs) { Name = "testpackage", Version = "1.0.2" };
